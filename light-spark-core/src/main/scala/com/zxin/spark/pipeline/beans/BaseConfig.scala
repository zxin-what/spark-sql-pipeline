...
import com.zxin.spark.pipeline.uitils.{ConfigException, Logging}
...
class BaseConfig extends Logging with Serializable {
   ...
   */
  def validateNoneIsBlank(requireFields: String*): Unit = {
  ...
  def stepCheck(): Unit = {
    require(StringUtils.isNotBlank(this.sql), s"In node '${this.tag}', 'sql' is required in item!")
  }

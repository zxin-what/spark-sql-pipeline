...
import com.zxin.spark.pipeline.ConfigMapping
import com.zxin.spark.pipeline.beans._
...
import com.zxin.spark.pipeline.uitils.{AppUtil, ConfigException, DateUtil, ReflectUtils}
...
object AppConfigLoader extends ConfigLoader {
  ...
  def loadAppConfig(file: String): BusinessConfig = {
    ...
    val format = AppUtil.formatVariableStr(appConfigStr)
    var busConfig = JSON.parseObject(format, classOf[BusinessConfig])
    // check name and type
    configNameCheck(busConfig.inputs)
    configStepCheck(busConfig.processes)
    ...
    loggerVariables()
    // replace
    val variableReplaced = variableReplace(format, variables)
    // 变量替换后，重新生成 bean
  ...
  def loadDefaultConfig(): Unit = {
    ...
    if (null != stream) {
      ...
      SysConstants.SYS_SPARK_CONFIG.putAll(parseSparkConfStrings(busConfig.getEnvs.getSpark))
      SysConstants.SYS_DEFAULT_VARIABLES.putAll(busConfig.getConstansMap)

      AppConstants.variables.putAll(SysConstants.SYS_DEFAULT_VARIABLES)
  ...
  private def string2Config(string: String, bean: BusinessConfig): Unit = {
    ...
    boxItems(bean.inputs, jsonObject)
    boxItems(bean.processes, jsonObject)
    boxItems(bean.outputs, jsonObject)
    // 判断是否为 streaming
   ...
   */
  private def boxItems(items: java.util.List[_ <: BaseConfig], jsonObject: JSONObject): Unit = {
  ...
  private def configNameCheck[T <: BaseConfig](item: java.util.List[T]): Unit = {
    Option(item).filter(inputs => CollectionUtils.isNotEmpty(inputs))
      .foreach(inputs => {
        inputs.foreach(input => input.nameCheck())
      })
  }
   ...
   * @tparam T
   */
  private def configStepCheck[T <: BaseTransformConfig](item: java.util.List[T]): Unit = {
    Option(item).filter(steps => CollectionUtils.isNotEmpty(steps))
      .foreach(step => {
        step.foreach(input => input.step.stepCheck())
      })
  }

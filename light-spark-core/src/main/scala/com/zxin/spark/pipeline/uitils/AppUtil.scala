...
object AppUtil extends Logging {
  ...
  def regexFind(regex: String, process: BaseTransformConfig, rule: Boolean = false): List[(String, String, BaseConfig)] = {
    val group = if (rule) 1 else 0
    regexFind(regex, process.step.sql, group).map(item => (item, process.step.getName, process.step))
  }
